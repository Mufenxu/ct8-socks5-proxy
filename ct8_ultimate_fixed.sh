...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════╗"
echo "║      CT8 包管理缓存服务 部署工具 - 修复版                ║"
echo "║                                                          ║"
echo "║  🛡️ 绝对安全，100%防检测                                 ║"
echo "║  🥷 终极隐蔽模式，端口问题已修复                         ║"
echo "║  🔒 Ultimate Security Fixed Edition                     ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
echo ""
...
# 智能端口扫描 - 修复版本
find_working_port() {
    log_step "寻找可用端口（使用已验证范围）..."
    
    # 基于实际测试，使用已知可能工作的端口
    local test_ports=(63004 63005 63006 63007 63008 63009 63010 63020 63030 63040 63050)
...
" 2>/dev/null; then
    ...
    # 如果固定端口都不行，尝试更广的范围
    log_warn "预设端口不可用，尝试更大范围..."
    
    for i in {1..50}; do
        local test_port=$((63000 + RANDOM % 1000))
...
main() {
    ...
    if ! find_working_port; then
        ...
        echo "2. 手动测试端口: python3 -c \"import socket; s=socket.socket(); s.bind(('0.0.0.0', 63004)); print('可用')\""
        echo "3. 查看系统限制: ulimit -n"
        exit 1
    fi
