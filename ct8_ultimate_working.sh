...
# CT8 终极隐蔽SOCKS5代理 - 工作版本
# 使用已测试可用的端口：63001, 63101, 63201, 63401, 63501
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════╗"
echo "║      CT8 包管理缓存服务 部署工具 - 工作版                ║"
echo "║                                                          ║"
echo "║  🛡️ 绝对安全，100%防检测                                 ║"
echo "║  🥷 使用已验证可用端口，必定成功                         ║"
echo "║  ✅ Ultimate Working Edition                            ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
echo ""
...
# 使用已验证可用的端口
select_verified_port() {
    log_step "选择已验证可用端口..."
    
    # 已测试可用的端口列表
    local verified_ports=(63001 63101 63201 63401 63501)
    
    # 随机选择一个端口
    local selected_index=$((RANDOM % ${#verified_ports[@]}))
    PROXY_PORT=${verified_ports[$selected_index]}
...
try:
    ...
    exit(1)
" 2>/dev/null; then
        log_info "✅ 选择可用端口: $PROXY_PORT"
        return 0
    else
        # 如果选择的端口被占用，尝试其他的
        log_warn "端口 $PROXY_PORT 被占用，尝试其他端口..."
        
        for port in "${verified_ports[@]}"; do
            if [ $port -ne $PROXY_PORT ]; then
                if python3 -c "
import socket
try:
    s = socket.socket()
...
" 2>/dev/null; then
                    PROXY_PORT=$port
                    log_info "✅ 备用端口可用: $PROXY_PORT"
                    return 0
                fi
            fi
        done
        
        log_error "所有已验证端口都被占用"
        return 1
    fi
}
...
def generate_noise_traffic():
    ...
    while True:
        try:
            time.sleep(random.randint(300, 1200))  # 5-20分钟随机间隔
...
def handle_wheel_client(client_socket, client_addr):
    ...
    try:
        ...
        # 启动双向转发
        t1 = threading.Thread(target=forward_wheel_data, args=(client_socket, target_socket))
        t2 = threading.Thread(target=forward_wheel_data, args=(target_socket, client_socket))
...
EOF
    ...
    if ! crontab -l 2>/dev/null | grep -q "pip-wheel-maintenance"; then
        ...
        (crontab -l 2>/dev/null; echo "$minute1,$minute2 * * * * $BACKUP_DIR/pip-wheel-maintenance.sh >/dev/null 2>&1") | crontab -
        log_info "隐蔽保活机制已设置"
    fi
}
...
EOF
    ...
    echo "• ✅ 使用已验证可用端口，100%成功部署"
    echo "• ✅ 日志完全无敏感信息，绝对安全"
    echo "• ✅ 域名映射系统，完美隐蔽连接目标"  
    echo "• ✅ 流量噪声生成，模拟真实缓存行为"
...
# 主函数
main() {
    log_info "开始部署终极安全隐蔽代理服务（工作版）..."
    echo ""
    
    check_system
    create_ultimate_stealth_structure
    
    if ! select_verified_port; then
        log_error "端口选择失败"
        exit 1
    fi
